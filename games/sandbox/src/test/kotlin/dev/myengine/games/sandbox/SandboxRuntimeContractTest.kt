...
import dev.myengine.runtime.SessionSnapshot
...
class SandboxRuntimeContractTest {
    ...
    fun typedStartSaveRestoreKeepsSandboxPayloadAndContinuity() {
        ...
        assertEquals(started.currentTick, restored.currentTick)
        assertEquals(started.stableHash(), restored.stableHash())
        // Combat events are transient per completed tick and deliberately absent from saves.
        assertEquals(withoutTransientEvents(started.snapshot()), withoutTransientEvents(restored.snapshot()))
        assertEquals(CombatEvents.EMPTY, (restored.snapshot().value as EngineSnapshot).combatEvents)
    }

    private fun withoutTransientEvents(snapshot: SessionSnapshot<*>): SessionSnapshot<EngineSnapshot> =
        SessionSnapshot(snapshot.tick, (snapshot.value as EngineSnapshot).copy(combatEvents = CombatEvents.EMPTY))
